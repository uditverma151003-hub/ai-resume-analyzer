...
function JobDescriptionInput({ onAnalyze, isAnalyzing, error }) {
    ...
    <div className="w-full max-w-3xl mx-auto space-y-4">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <label htmlFor="job-description-input" className="text-base font-semibold text-white block">
        ...
        <textarea
          ...
          className="w-full p-4 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 placeholder-slate-500 text-sm focus:outline-none focus:border-indigo-500 transition-colors resize-y min-h-[140px]"
        />

        {error && (
          <div className="p-3 bg-rose-950/40 border border-rose-800/60 rounded-lg flex items-center justify-between text-rose-200 text-sm">
            <span>{error}</span>
            <button
              onClick={() => onAnalyze(jobDescription.trim())}
              className="px-3 py-1 bg-rose-900 hover:bg-rose-800 text-white rounded text-xs font-medium transition-colors"
            >
              Try again
            </button>
          </div>
