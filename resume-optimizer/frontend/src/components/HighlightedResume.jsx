    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
      <div className="bg-slate-950/90 px-5 py-3 border-b border-slate-800 flex items-center justify-between">
        ...
        <span className="text-xs text-indigo-400 font-medium">
          Click any line to AI rewrite
        </span>
      </div>

      <div className="p-4 bg-slate-900/90 text-slate-300 font-mono text-xs sm:text-sm leading-relaxed max-h-[500px] overflow-y-auto space-y-1">
        {lines.map((line, lineIdx) => {
          ...
          return (
            <div
              ...
              className={`group relative rounded px-3 py-1.5 transition-all duration-150 flex items-start justify-between gap-3 ${
                isEligibleForRewrite
                  ? 'hover:bg-indigo-950/40 cursor-pointer border border-transparent hover:border-indigo-500/40'
                  : 'cursor-default'
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {isEligibleForRewrite && (
                  <span className="opacity-0 group-hover:opacity-100 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-indigo-600 text-white shadow-sm transition-opacity">
                    AI Rewrite
                  </span>
                )}
              </div>
